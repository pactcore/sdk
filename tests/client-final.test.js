...
describe("PactSdk - Final alignment - Anti-spam", () => {
    it("checkAntiSpam -> POST /anti-spam/check", async () => {
        ...
        expect(captured[0].body.participantId).toBe("worker_1");
        expect(captured[0].body.action).toBe("task_creation");
    });
    it("recordAntiSpamAction -> POST /anti-spam/record", async () => {
        ...
        expect(captured[0].body.participantId).toBe("worker_1");
        expect(captured[0].body.action).toBe("data_listing");
    });
...
describe("PactSdk - Final alignment - Disputes", () => {
    it("createDispute -> POST /disputes", async () => {
        ...
        expect(captured[0].body.missionId).toBe("mission_1");
    });
    ...
    it("submitDisputeEvidence -> POST /disputes/:id/evidence", async () => {
        ...
        expect(captured[0].body.submitterId).toBe("worker_2");
    });
    it("voteOnDispute -> POST /disputes/:id/vote", async () => {
        ...
        expect(captured[0].body.jurorId).toBe("jury_1");
        expect(captured[0].body.vote).toBe("uphold");
    });
