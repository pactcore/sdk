...
describe("PactSdk - Route sync coverage", () => {
  it("registerParticipant -> POST /id/participants", async () => {
    ...
    expect((captured[0].body as Record<string, unknown>).displayName).toBe("Worker One");
  });
  ...
  it("createTask -> POST /tasks", async () => {
    ...
    expect((captured[0].body as Record<string, unknown>).title).toBe("Inspect route");
    expect((captured[0].body as Record<string, unknown>).stakeCents).toBe(200);
  });
  ...
  it("assignTask -> POST /tasks/:id/assign", async () => {
    ...
    expect((captured[0].body as Record<string, unknown>).workerId).toBe("worker_1");
  });
  ...
  it("submitTask -> POST /tasks/:id/submit", async () => {
    ...
    expect((captured[0].body as Record<string, unknown>).summary).toBe("Work complete");
  });
