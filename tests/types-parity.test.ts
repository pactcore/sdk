...
import type {
  ...
  ZKArtifactManifest,
  ZKAdapterHealthReport,
  ZKAdapterHealthSummary,
  ZKBridgeRuntimeInfo,
  ZKProof,
} from "../src/types";
import type {
...
describe("Type parity contracts", () => {
  ...
  it("models Appendix C adapter health metadata", () => {
    const adapterHealth: ZKAdapterHealthReport = {
    ...
    const summary: ZKAdapterHealthSummary = {
      status: "degraded",
      checkedAt: 1700000000200,
      runtimeVersion: "0.2.0",
  ...
  it("models live onchain indexer contracts", async () => {
    ...
    const event: OnchainIndexerSyncEvent = {
    ...
    const hook: OnchainIndexerSyncHook = async (input) => {
