...
describe("PactSdk - Batch 35 - Onchain finality", () => {
  ...
  it("listOnchainTransactions -> GET /onchain/finality/transactions with filters", async () => {
    ...
    expect(captured[0].url).toBe(
      "https://api.pact/onchain/finality/transactions?status=finalized&operation=governance_proposal_create&participantId=council-1&proposalId=proposal-1&epoch=7&referenceId=proposal-1&cursor=0&limit=10",
    );
  });
  ...
  it("trackOnchainTransaction -> POST /onchain/finality/transactions", async () => {
    ...
    expect((captured[0].body as Record<string, unknown>).txId).toBe("0xtx-track-1");
    expect((captured[0].body as Record<string, unknown>).proposalId).toBe("proposal-2");
  });
  ...
  it("recordOnchainTransactionInclusion -> POST /onchain/finality/transactions/:txId/inclusion", async () => {
    ...
    expect((captured[0].body as Record<string, unknown>).txId).toBeUndefined();
    expect((captured[0].body as Record<string, unknown>).blockNumber).toBe(77);
  });
  ...
  it("recordCanonicalBlock -> POST /onchain/finality/blocks/canonical", async () => {
    ...
    expect((captured[0].body as Record<string, unknown>).blockNumber).toBe(77);
    expect((captured[0].body as Record<string, unknown>).blockHash).toBe("0xblock-77");
  });
});
