...
describe("SDK settlement runtime helpers", () => {
    it("executes settlement and queries reconciliation/audit records through core APIs", async () => {
        ...
        const execution = await sdk.executeSettlement(buildSettlementExecutionRequest(model, {
            settlementId: "settlement-sdk-1",
            idempotencyKey: "sdk-settlement-1",
        }));
        ...
        expect((captured[0]?.body).idempotencyKey).toBe("sdk-settlement-1");
        ...
        expect(captured[1]?.url).toBe("https://core.pact/economics/settlements/records?settlementId=settlement-sdk-1&rail=llm_metering");
        ...
        expect(captured[2]?.url).toBe("https://core.pact/economics/settlements/records/page?settlementId=settlement-sdk-1&status=reconciled&reconciledBy=auditor-1&cursor=0&limit=1");
        ...
        expect(captured[3]?.url).toBe("https://core.pact/economics/settlements/records/page?settlementId=settlement-sdk-1&status=reconciled&limit=2");
        const replay = await sdk.replaySettlementRecordLifecycle({ fromOffset: 0, limit: 10 });
        expect(replay.entries.length).toBe(1);
        expect(captured[4]?.url).toBe("https://core.pact/economics/settlements/records/replay?fromOffset=0&limit=10");
