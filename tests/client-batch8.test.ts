...
describe("PactSdk - Batch 8 - Token economics", () => {
  ...
  it("calculateTokenApy -> POST /economics/token/apy", async () => {
    ...
    expect((captured[0].body as Record<string, unknown>).totalStaked).toBe(1_000_000);
    expect((captured[0].body as Record<string, unknown>).emissionRate).toBe(150_000);
  });
  ...
  it("calculateTokenBurnRate -> POST /economics/token/burn-rate", async () => {
    ...
    expect((captured[0].body as Record<string, unknown>).transactionVolume).toBe(100_000);
    expect((captured[0].body as Record<string, unknown>).burnPercent).toBe(3);
  });
});
...
describe("PactSdk - Batch 8 - Role/participant matrix", () => {
  ...
  it("checkRoleAction -> POST /roles/check-action", async () => {
    ...
    expect((captured[0].body as Record<string, unknown>).role).toBe("task_issuer");
    expect((captured[0].body as Record<string, unknown>).module).toBe("tasks");
    expect((captured[0].body as Record<string, unknown>).action).toBe("create");
  });
  ...
  it("getParticipantMatrixCategory -> POST /participants/matrix/category", async () => {
    ...
    expect((captured[0].body as Record<string, unknown>).type).toBe("organization");
    expect((captured[0].body as Record<string, unknown>).isAgent).toBe(true);
  });
});
